	...
	return (
		<AnimatePresence>
			{showNav && (
				...
				>
					{isMobile ? (
						<MobileNavigation handleNavClick={handleNavClick} />
					) : (
						<DesktopNavigation handleNavClick={handleNavClick} />
					)}
				</motion.div>
			)}
		</AnimatePresence>
...
function DesktopNavigation({ handleNavClick }: { handleNavClick: (e: React.MouseEvent<HTMLAnchorElement>) => void }) {
...
function MobileNavigation({ handleNavClick }: { handleNavClick: (e: React.MouseEvent<HTMLAnchorElement>) => void }) {
...
function NavItem({ item, onClick }: NavItemProps) {
	return (
		<Link
			href={item.href}
			className={cn("text-sm font-medium text-white/70 transition-colors hover:text-white")}
			onClick={onClick}
		>
			{item.name}
		</Link>
